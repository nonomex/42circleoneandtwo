...
/*   Updated: 2023/11/13 14:24:37 by aammirat         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	destroymutex(t_table *table)
{
	...
	if (table->whereinit - table->nb_philo >= 7)
	{
		i = -1;
	...
	if (table->whereinit - (table->nb_philo * 2) >= 8)
	{
		i = -1;
		while (++i < table->nb_philo && \
...
void	freephilo(t_table *table)
{
	...
	if (table->whereinit >= 1)
		free(table->fork);
	if (table->whereinit - table->nb_philo >= 2)
